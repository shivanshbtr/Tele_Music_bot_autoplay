...
class MusicAPIService:
    ...
    async def get_track_by_id(self, track_id: str) -> Optional[Track]:
        try:
            results = ytmusic.search(track_id, filter="songs", limit=1)
            if results:
    ...
    async def get_similar_tracks(self, artist: str, title: str, limit: int = 5) -> List[Track]:
        try:
            results = ytmusic.search(f"{artist} {title}", filter="songs", limit=1)
            if not results:
                return []
            video_id = results[0].get("videoId")
            if not video_id:
                return []
            radio = ytmusic.get_watch_playlist(videoId=video_id, limit=limit + 1)
    ...
    async def get_similar_artists(self, artist: str, limit: int = 5) -> List[str]:
        try:
            results = ytmusic.search(artist, filter="artists", limit=1)
            if not results:
                return []
            artist_id = results[0].get("browseId")
            if not artist_id:
